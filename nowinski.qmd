...
Usunąłem zmienną `speed` ze zbioru danych.
...
Stanowi to `r  pct_miss(df$rating_weather)`% obserwacji.
...
```{r}
imputation_1_miss <- imputation_1[indexes_missing_values,]
imputation_2_miss <- imputation_2[indexes_missing_values,]
imputation_3_miss <- imputation_3[indexes_missing_values,]
imputation_4_miss <- imputation_4[indexes_missing_values,]
imputation_5_miss <- imputation_5[indexes_missing_values,]
```
...
ggplot() +
  geom_bar(data = df,
               aes(x = rating_bus, fill = "original"),
               alpha = 0.5) +
  geom_bar(data = imputation_2, 
               aes(x=rating_bus, fill = "imputed"),
               alpha = 0.5) +
...
ggplot() +
  geom_bar(data = df,
               aes(x = rating_weather, fill = "original"),
               alpha = 0.5) +
  geom_bar(data = imputation_2, 
               aes(x=rating_weather, fill = "imputed"),
               alpha = 0.5) +
  scale_fill_manual(values = c("original" = 'blue',
                               "imputed" = 'red'),
                    name = "Legend")
...
nie wiem jak sprawdzić poprawność i jaki wybrać
...
```{r eval = FALSE}
split <- initial_split(df[,c(-4,-5)])
train <- training(split)
test <- testing(split)
rec <- recipe(rating ~ ., data = train)
rf <- rand_forest(mode = "classification", mtry = 3, trees = 1000) %>% 
  set_engine("ranger")
wflow <- workflow() %>% 
  add_recipe(rec) %>% 
  add_model(rf)
...
Model z imputacją danych


```{r}
split <- initial_split(imputation_2)
train <- training(split)
test <- testing(split)
rec <- recipe(rating ~ ., data = train)
